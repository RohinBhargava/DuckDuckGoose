...
class State:
    def __init__(self, id: int, hatchlings: int, partition: int, election_wait: float) -> None:
        self.id: int = id
        ...
        self.service_discovery_connection = RedisServiceDiscovery(self.service_discovery_partition)
        self.service_discovery_connection.write(f'{APP_HOST}:{APP_BASE_PORT + self.id}', f'{APP_HOST}:{APP_BASE_PORT + self.id}')
